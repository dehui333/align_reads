...
class Aligner {
  ...
  private:
    ...
    struct align_boundary {
        ...
        bool contained;
        align_boundary(std::uint32_t start, std::uint32_t end, bool contained)
                : align_start(start), align_end(end), contained(contained) {};
    };
    ...
    struct align_overlapping_result {
        ...
        align_overlapping_result(align_overlapping_result &&r)
                : alignment(std::move(r.alignment)), infos(std::move(r.infos)),
    ...
    static align_result align_to_target_clip(std::vector<std::string> &queries,
                                             std::string &target,
    ...
    static align_result multi_align(std::vector<std::string> &queries,
                                    std::string &target,
                                    std::vector<std::pair<std::uint32_t, std::uint32_t>> &clips,
                                    std::vector<EdlibAlignResult> &edlib_results,
                                    bool has_hap = false);
